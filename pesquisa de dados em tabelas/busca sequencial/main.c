...
int buscaSequencial(universidade aluno[], int mat) {
    int i, x;
    for (i=0; i<10; i++)
    {
        ...
        {
            return i; //retorna a posição do registro na tabela
        }
    }
    return -1; //caso não esteja na tabela, retorna -1
}

void imprimeRegistro(int r, universidade aluno[]) {
    printf ("Matricula: %d \n", aluno[r].matricula);
    printf ("Nome: %s \n", aluno[r].nome);
    printf ("Nota 1: %.1f \n", aluno[r].nota1);
    printf ("Nota 2: %.1f \n", aluno[r].nota2);
...
main() {
    ...
    scanf("%d", &mat);
    r = buscaSequencial(aluno, mat);
    if (r == -1) {
        printf("Aluno nao encontrado\n");
    } else {
    	imprimeRegistro(r, aluno);
    }
    system("pause");
}
